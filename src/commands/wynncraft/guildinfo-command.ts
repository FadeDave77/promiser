...
export default class WynnGuild extends Command {
    ...
    public async exec(message: Message, {guild}: {guild: string}): Promise<Message> {
        ...
        .addField('Rank Names', data.members.map(u=> {switch (u.rank) {case 'OWNER': return `\`*****${u.name}\``; case 'CHIEF': return `\`**** ${u.name}\``; case 'STRATEGIST': return `\`***  ${u.name}\``; case 'CAPTAIN': return `\`**   ${u.name}\``; case 'RECRUITER': return `\`*    ${u.name}\``; case 'RECRUIT': return `\`     ${u.name}\``;}}).slice(Math.floor(data.members.length / 2)), true)
