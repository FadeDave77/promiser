...
export default {
	async end(giveawayRepo: Repository<Giveaways>, msg: Message): Promise<any> {
		await msg.fetch().catch(()=> null);
		const entry = await giveawayRepo.findOne({ message: msg.id }); if (!entry) return;
		let winamount = entry.winners;
		const reaction: MessageReaction | undefined = await msg?.reactions?.cache?.filter(r => r.emoji.name === '🎉')?.first()?.fetch(); if (!reaction) return;
		...
		const embed: MessageEmbed | undefined = msg?.embeds[0]; if (!embed) return;
