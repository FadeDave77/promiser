...
export default class MessageDeleteListener extends Listener {
    ...
    public async exec(message: Message): Promise<Message | undefined> {
        ...
        const logs = await message.guild?.fetchAuditLogs({ type: 72, limit: 6 }); if (!logs) return;
        const entry = logs.entries.find((a: GuildAuditLogsEntry) => (a.target as any).id == message.author.id && (a.extra as any).channel.id === message.channel.id && Date.now() - a.createdTimestamp < 20000); if (!entry) return;
        ...
        if (executor) {
            embed.addField('Executor:', `${executor} *${executortag}* \`${executorid}\``)
